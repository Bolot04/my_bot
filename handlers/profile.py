...
from keyboards.inline_bottons import like_dislike_keyboard, my_profile_keyboard
...
async def my_profile_call(call: types.CallbackQuery):
    ...
    if profile:
        ...
        await bot.send_message(
            chat_id=call.from_user.id,
            text='You have not profile, please Register'
        )


async def random_profile_call(call: types.CallbackQuery):
    db = Database()
    profile = db.sql_select_filter_profile(
        tg_id=call.from_user.id
    )
    if profile:
        ...
        with open(random_profile['photo'], 'rb') as photo:
            await bot.send_photo(
                ...
                caption=PROFILE_TEXT.format(
                    ...
                    number=random_profile['number'],
                    street=random_profile['street']
                ),
                parse_mode=types.ParseMode.MARKDOWN,
        ...
        await bot.send_message(
            chat_id=call.from_user.id,
            text='There is no profile that you have not liked'
        )
