...
class Database:
    ...
    def sql_insert_profile(self, tg_id, nickname, bio, age, gender, number, street, photo):
        self.cursor.execute(
            sql_queries.INSERT_PROFILE_QUERY,
            (None, tg_id, nickname, bio, age, gender, number, street, photo)
        )
        self.connection.commit()
    ...
    def sql_select_filter_profile(self, tg_id):
        self.cursor.row_factory = lambda cursor, row: {
            "id": row[0],
            "telegram_id": row[1],
            "nickname": row[2],
    ...
    def sql_update_profile(self, nickname, bio, age, gender, number, street, photo, tg_id):
        self.cursor.execute(
            sql_queries.UPDATE_PROFILE_QUERY,
            (nickname, bio, age, gender, number, street, photo, tg_id)
        )
        self.connection.commit()
    ...
    def sql_update_user_link(self, link, tg_id):
        self.cursor.execute(
            sql_queries.UPDATE_LINK_USER_QUERY,
            (link, tg_id,)
        )
        self.connection.commit()
    ...
    def sql_select_user_by_link(self, link):
        ...
        return self.cursor.execute(
            sql_queries.SELECT_BY_LINK_QUERY,
            (link,)
        ).fetchone()
